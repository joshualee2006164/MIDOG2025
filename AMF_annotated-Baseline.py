...
class AMFLoader(Dataset):
    ...
    def __getitem__(self, index):
        ...
        atypical = str(row['label']).strip().upper() == "AMF"
        image_name = row['image_file']

        image_path = os.path.join(self.root, 'LabelStudioAMF_annotated', image_name)
...
def main(args):
    ...
    data_path = os.path.join(os.getcwd(), "AMF_Annotated")
    path = os.path.join(os.getcwd(), "AMF_Annotated", "AMF_annotation.csv")
    df = pd.read_csv(path,header=0)
    df = df[df['label'].isin(['AMF', 'MF'])]
    ...
    train_data = AMFLoader(train_df, data_path, data_transform)
    val_data = AMFLoader(val_df, data_path, data_transform)
    test_data = AMFLoader(test_df, data_path, data_transform)
