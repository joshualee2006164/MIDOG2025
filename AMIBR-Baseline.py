...
class AMIBRLoader(Dataset):
    ...
    def __getitem__(self, index):
        ...
        dataset = row['dataset']
        uid = row['uid']
        atypical = str(row['majority_atypical']).strip().upper() == "TRUE"

        image_name = dataset + '_' + str(uid) + '.png'

        if atypical:
            img_path = os.path.join(self.root,'patches', 'atypical')
            
        else:
            img_path = os.path.join(self.root,'patches', 'normal')
...
def main(args):
    ...
    data_path = os.path.join(os.getcwd(), "AMi-Br-main")
    path = os.path.join(os.getcwd(), "AMi-Br-main", "AMI-BR.csv")
    ...
    train_data = AMIBRLoader(train_df, data_path, data_transform)
    val_data = AMIBRLoader(val_df, data_path, data_transform)
    test_data = AMIBRLoader(test_df, data_path, data_transform)
...
output_filename = f"AMIBR-{timestamp}.txt"
